...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/02/25 13:39:00 by eyevresh          #+#    #+#             */
/*   Updated: 2018/02/25 13:39:00 by eyevresh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char		*ft_dtoa(double value, int precision, int flag)
{
	...
	tmp = value < 0 ? 1 : 0;
	if (precision == 0)
		result = ft_strnew(ft_llen_base((long)value, 10) + tmp + flag);
	else
		result = ft_strnew(ft_llen_base((long)value, 10) + precision + 1 + tmp);
	if (!(i = 0) && tmp)
		result[i++] = '-';
	if (precision == 0)
