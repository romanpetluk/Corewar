...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/03/02 18:43:00 by eyevresh          #+#    #+#             */
/*   Updated: 2018/03/02 18:43:00 by eyevresh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*fill_floating_part(char *res, int prec, double tmp, int c_case)
{
	char	*base;
	int		i;
	int		sign;
	...
	sign = res[0] == '-' ? 1 : 0;
	base = c_case == 1 ? "0123456789ABCDEF" : "0123456789abcdef";
	i = ft_strlen(res);
	while (tmp *= 16)
	{
...
char		*ft_dtoa_format_a(double value, int precision, int c_case, int f)
{
	char	*res;
	double	tmp;
	int		exponent;
	...
	res = ft_strnew(sign + 4 + f);
	sign == 1 ? res[0] = '-' : 0;
	res = c_case == 1 ? ft_strcat(res, "0X1") : ft_strcat(res, "0x1");
	tmp = value / ft_power(2, exponent);
	if (precision != 0)
	{
		res[ft_strlen(res)] = '.';
		res = fill_floating_part(res, precision, tmp - (long)tmp, c_case);
