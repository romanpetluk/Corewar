...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/03/02 15:28:00 by eyevresh          #+#    #+#             */
/*   Updated: 2018/03/02 15:28:00 by eyevresh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_format(long double value, int precision)
{
	int format;
	int exponent;
	int counter;
...
static char	*handle_f_format(long double value, int precision, int f)
{
	int		sign;
	long	float_part;
	char	*result;
...
char		*ft_ldtoa_format_g(long double value, int prec, int c_case, int f)
{
	char	*result;

	prec = prec < 0 ? 6 : prec;
	prec = prec == 0 ? 1 : prec;
	if (get_format(value, prec))
	{
		result = ft_ldtoa_format_e(value, prec - 1, c_case, f);
		if (!f)
			remove_trailing_zeroes(result, c_case);
	}
	else
		result = handle_f_format(value, prec, f);
	return (result);
}
