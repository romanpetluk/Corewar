...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/02/25 17:14:00 by eyevresh          #+#    #+#             */
/*   Updated: 2018/02/25 17:14:00 by eyevresh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_exponent(double *value, int precision)
{
	int exponent;
...
char		*ft_dtoa_format_e(double value, int precision, int c_case, int f)
{
	char	*result;
	size_t	i;
	int		exp;
	...
	tmp = value < 0 ? 1 : 0;
	result = ft_strnew(precision + 1 + tmp + 5);
	if (!(i = 0) && tmp)
		result[i++] = '-';
	if (!(exp = 0) && ((long)value == 0 || ft_llen_base((long)value, 10) > 1))
		exp = get_exponent(&value, 99);
	if (precision == 0)
		if (ft_get_float_part(value, 1) > 4)
			value += value < 0 ? -1.0 : 1.0;
	fill_int_part((long)value, "0123456789", result, &i);
	if (precision != 0)
	{
		result[i++] = '.';
		tmp = ft_get_rounded_float_value(value, precision);
		i += (size_t)fill_float_part(tmp, result + i, precision);
	}
